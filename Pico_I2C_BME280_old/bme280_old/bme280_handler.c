...
{
    if(i2c_write_blocking(i2c_default, BME280_I2C_ADDR_PRIM, &reg_addr, 1, false) < 0) return -1;
    if(i2c_read_blocking(i2c_default, BME280_I2C_ADDR_PRIM, data, len, false) < 0) return -1;
...
int8_t bme280h_user_i2c_write(uint8_t id, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
    ...
    if(i2c_write_blocking(i2c_default, BME280_I2C_ADDR_PRIM, (uint8_t*)buf, len + 1, false) < 0) return -1;
